...
const DIAGNOSTICS_CONSOLE_STORAGE_KEY = "tome.debug.lifecycle.console";
...
declare global {
  ...
  // eslint-disable-next-line no-var
  var __TOME_FORCE_DIAGNOSTICS_CONSOLE__: boolean | undefined;
}

interface DiagnosticsWindow extends Window {
  __TOME_DIAGNOSTICS__?: DiagnosticEntry[];
  __TOME_PRINT_DIAGNOSTICS__?: (limit?: number) => DiagnosticEntry[];
  __TOME_CLEAR_DIAGNOSTICS__?: () => void;
}
...
function readDiagnosticsConsolePreference(): boolean | null {
  if (typeof window === "undefined") {
    return null;
  }

  try {
    const stored = window.localStorage.getItem(DIAGNOSTICS_CONSOLE_STORAGE_KEY);
    if (stored === "1") {
      return true;
    }
    if (stored === "0") {
...
export function isDiagnosticsConsoleEnabled(): boolean {
  if (globalThis.__TOME_FORCE_DIAGNOSTICS_CONSOLE__ === true) {
    return true;
  }
  if (globalThis.__TOME_FORCE_DIAGNOSTICS_CONSOLE__ === false) {
    return false;
  }

  const storedPreference = readDiagnosticsConsolePreference();
  if (storedPreference !== null) {
    return storedPreference;
  }
...
export function logDiagnostics(
  ...
  try {
    ...
    diagnosticsWindow.__TOME_PRINT_DIAGNOSTICS__ = (limit = 50) => {
      const slice = entries.slice(-Math.max(1, limit));
      console.table(slice);
      return slice;
    };
    diagnosticsWindow.__TOME_CLEAR_DIAGNOSTICS__ = () => {
      diagnosticsWindow.__TOME_DIAGNOSTICS__ = [];
    };
...
export function clearDiagnosticsForTest() {
  ...
  delete (window as DiagnosticsWindow).__TOME_PRINT_DIAGNOSTICS__;
  delete (window as DiagnosticsWindow).__TOME_CLEAR_DIAGNOSTICS__;
  delete globalThis.__TOME_FORCE_DIAGNOSTICS__;
  delete globalThis.__TOME_FORCE_DIAGNOSTICS_CONSOLE__;
}
