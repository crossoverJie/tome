...
- **命令语法高亮实现方案**：
  ...
  - 当前实现：前端使用 CodeMirror + 自定义文本解析提取 command/path token，Rust 侧缓存 PATH 可执行文件与 zsh builtin，异步返回验证结果；无效 token 以红色波浪线提示
...
- **全屏模式支持标准编辑快捷键：Cmd+Backspace (Cmd+Del) 删除光标到行首**
...
- [x] 前端：全屏模式支持 Cmd+Backspace 删除到行首
